...
class OshmsApp:
    ...
    ROADMAP = [
        ...
        ("v3.3", "시장 심리 지표 통합 (공포/탐욕 지수→매수/매도 타이밍 조절)", False),
        ("v3.3", "동적 비중 조절 (확신도 높을수록 자금 집중 배분)", False),
        ("v3.4", "섹터 모멘텀 로테이션 (상위 섹터→집중 매매, 하위 섹터→회피)", False),
        ("v3.4", "AI 뉴스 분석 (실시간 공시/뉴스→매매 신호 자동 반영)", False),
    ]
